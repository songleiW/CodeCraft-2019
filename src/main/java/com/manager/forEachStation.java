...
public class forEachStation {
		...
		for(Road roadTemp:station.roadList)//依次按顺序处理路
		{
			while(true)//一直处理到本路没有车可以处理
			{
				...
				if(successCrossOneCar(car, station))//处理此车  如果有冲突车辆　则下一轮在调度
				{
					...
					StartCars.startNewCars(true);//优先车辆上路
				}
				else 
				{
					break;
	...
	/*******************车到达终点站 但也许要参与优先级排序***********************************/
		if(car.getTo().equals(station.getId())) {//车到达终点站  按直行处理
			...
			//不是优先车辆
			Car crashCar;		//冲突车辆
			int dirNow=station.toRoad.indexOf(car.nowOnChannel.ID); //当前车辆路口所处的位置
			//先处理直行冲突车辆
			String crashRoadId=station.toRoad.get(Math.floorMod(dirNow-1, 4));//右转冲突车辆
		...
		if(dir.equals("D"))//直行 
		{
			...
			//不是优先车辆
			dirNow=station.toRoad.indexOf(car.nowOnChannel.ID);
	...
	public static Car getFirstCarOfRoad(Station station,Road road)//获得道路的第一优先级可以动的车
	{
		...
		for(Channel channel:channels)
		{
			...
			{
				if(channel.carPortList.get(0).isEndThisTime())//已经结束这一周期
				{
					continue;
				}
				if(channel.carPortList.get(0).isPriority())//如果是优先车辆
				{
					priorityCars.add(channel.carPortList.get(0));//就加入优先队列
				}
				else {//非优先车辆
		...
		return car!=null?car:getFirstCarOfList(cars);//如果在优先队列内找到了车就范挥着个车 否则遍历非优先队列
	}
